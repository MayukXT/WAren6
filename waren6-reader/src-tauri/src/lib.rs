...
fn sha256_path(path: &Path) -> Result<String, String> {
    ...
    loop {
        let n = file.read(&mut buf).map_err(|e| format!("Failed to read file for hashing: {e}"))?;
        if n == 0 {
            break;
        }
        hasher.update(&buf[..n]);
...
"#;
    ...
    fs::write(helper_path, script).map_err(|e| format!("Failed to write portable update helper: {e}"))?;
    let _ = current_exe;
    let _ = new_exe;
    let _ = backup_exe;
    let _ = log_path;
    ...
    F: FnOnce(&Connection) -> Result<T, String>,
{
    ...
    let conn = guard.as_ref().ok_or("No database loaded. Please open a unified_whatsapp.db file.")?;
    f(conn)
}
...
fn sha256_file(path: &str) -> Result<String, String> {
    ...
    let mut buf = [0u8; 1024 * 1024];
    loop {
        let n = file.read(&mut buf).map_err(|e| format!("Failed to read DB for hashing: {e}"))?;
        if n == 0 {
            break;
        }
        hasher.update(&buf[..n]);
...
fn get_install_type() -> Result<String, String> {
    let exe_path = std::env::current_exe().map_err(|e| format!("Failed to resolve current executable: {e}"))?;
...
async fn install_portable_update(
    ...
    if detect_install_type_for_path(&exe_path) != "portable" {
        return Err("Portable replacement is only available for the portable Reader EXE.".to_string());
    }

    let safety = portable_update_safety_for_path(&exe_path, parent_folder_is_writable(&exe_path));
    if !safety.safe {
        return Err(safety.reason.unwrap_or_else(|| "Current Reader EXE cannot be replaced safely.".to_string()));
    }
    ...
    fs::create_dir_all(&update_dir).map_err(|e| format!("Failed to create update temp folder: {e}"))?;
    ...
    tauri::async_runtime::spawn_blocking(move || -> Result<(), String> {
        ...
        if !response.status().is_success() {
            return Err(format!("Portable Reader update download returned HTTP {}", response.status()));
        }
    ...
    if actual_sha256 != expected_sha256 {
        ...
        return Err("Downloaded portable Reader hash did not match WAren6-Reader-latest.json.".to_string());
    }
...
fn cache_path_for<R: tauri::Runtime>(app: &tauri::AppHandle<R>, db_hash: &str) -> Result<String, String> {
    let dir = app
        .path()
        .app_data_dir()
    ...
    Ok(dir.join(format!("{db_hash}.sqlite")).to_string_lossy().into_owned())
}

fn build_search_cache<R: tauri::Runtime>(app: &tauri::AppHandle<R>, source_path: &str, source_hash: &str, source: &Connection) -> Result<Connection, String> {
    let cache_path = cache_path_for(app, source_hash)?;
    build_search_cache_at_path(cache_path, source_path, source_hash, source)
}

fn build_search_cache_at_path(cache_path: String, source_path: &str, source_hash: &str, source: &Connection) -> Result<Connection, String> {
    let mut cache = Connection::open(cache_path).map_err(|e| format!("Failed to open search cache: {e}"))?;
    cache.execute_batch(
        "
    ...
    let existing_hash: Option<String> = cache
        .query_row("SELECT value FROM cache_metadata WHERE key='source_sha256'", [], |r| r.get(0))
        .optional()
        .map_err(|e| e.to_string())?;
    let existing_path: Option<String> = cache
        .query_row("SELECT value FROM cache_metadata WHERE key='source_path'", [], |r| r.get(0))
        .optional()
        .map_err(|e| e.to_string())?;

    if existing_hash.as_deref() != Some(source_hash) || existing_path.as_deref() != Some(source_path) {
        ...
        tx.execute("INSERT OR REPLACE INTO cache_metadata(key, value) VALUES ('source_sha256', ?)", [source_hash])
            .map_err(|e| e.to_string())?;
        tx.execute("INSERT OR REPLACE INTO cache_metadata(key, value) VALUES ('source_path', ?)", [source_path])
            .map_err(|e| e.to_string())?;
...
fn table_column_exists(conn: &Connection, table_name: &str, column_name: &str) -> Result<bool, String> {
...
fn message_select_sql_owned(conn: &Connection) -> Result<String, String> {
    ...
    let has_edit_columns = ["is_edited", "edited_at", "edit_count", "edit_history_status"]
        .iter()
        .map(|column| table_column_exists(conn, "messages", column))
        .collect::<Result<Vec<_>, _>>()?
        .into_iter()
    ...
    ", message_select_sql(), edit_columns, visible_messages))
}
...
fn validate_db(state: State<AppState>) -> Result<ExtractionInfo, String> {
    with_db(&state, |conn| {
        ...
        .optional()
        .map_err(|e| e.to_string())?
        .ok_or("This does not appear to be a unified_whatsapp.db file (messages table missing).".to_string())?;
...
fn get_messages(chat_jid: String, state: State<AppState>) -> Result<Vec<Message>, String> {
    with_db(&state, |conn| {
        ...
        let msgs = stmt.query_map([chat_jid], |r| {
            message_from_row(r)
        })
        .map_err(|e| e.to_string())?
        .filter_map(|r| r.ok())
...
#[tauri::command]
fn get_contact_info(chat_jid: String, state: State<AppState>) -> Result<Option<ContactInfo>, String> {
    with_db(&state, |conn| {
...
#[tauri::command]
fn get_group_participants(group_jid: String, state: State<AppState>) -> Result<Vec<GroupParticipant>, String> {
    with_db(&state, |conn| {
        ...
        let participants = stmt.query_map([&group_jid], |r| {
            Ok(GroupParticipant {
                group_jid:         r.get(0)?,
...
#[tauri::command]
fn get_group_info(group_jid: String, state: State<AppState>) -> Result<Option<serde_json::Value>, String> {
    with_db(&state, |conn| {
        let result = conn.query_row(
...
#[tauri::command]
fn get_contact_picture(_chat_jid: String, _state: State<AppState>) -> Result<Option<String>, String> {
...
fn resolve_participant_name_from_conn(conn: &Connection, jid: &str) -> Result<Option<ResolvedParticipant>, String> {
    let raw = jid
        .split('@')
        .next()
        .unwrap_or(jid)
    ...
    if table_exists(conn, "messages")? {
        let sender_fallback = conn.query_row(
            "SELECT
...
#[tauri::command]
fn resolve_participant_name(jid: String, state: State<AppState>) -> Result<Option<ResolvedParticipant>, String> {
    with_db(&state, |conn| resolve_participant_name_from_conn(conn, &jid))
}
...
fn search_messages(query: String, state: State<AppState>) -> Result<Vec<SearchResult>, String> {
    with_db(&state, |conn| {
        ...
        if let Some(cache_query) = fts_query.clone() {
            let rowids = {
                ...
                if let Some(cache) = cache_guard.as_ref() {
                    let mut stmt = cache
                        ...
                        .map_err(|e| e.to_string())?;
                    let rows = stmt.query_map([cache_query], |r| r.get::<_, i64>(0))
                        .map_err(|e| e.to_string())?
                        .filter_map(|r| r.ok())
                        .collect::<Vec<_>>();
                    rows
            ...
            if !rowids.is_empty() {
                ...
                let sql = format!("{}
                    ...
                    LIMIT 200
                ", select_sql);
                let params: Vec<&dyn rusqlite::types::ToSql> = rowids.iter().map(|id| id as &dyn rusqlite::types::ToSql).collect();
                let mut stmt = conn.prepare(&sql).map_err(|e| e.to_string())?;
                let results = stmt.query_map(params.as_slice(), |r| {
                    Ok(SearchResult {
        ...
        let results: Result<Vec<SearchResult>, String> = stmt.query_map([search_param.clone()], |r| {
            Ok(SearchResult {
                msg: message_from_row(r)?,
                chat_name: r.get(30)?,
            })
        ...
        match results {
            ...
            _ if fts_available => {
                ...
                let fallback_results = stmt2.query_map([pattern], |r| {
                    Ok(SearchResult {
                        msg: message_from_row(r)?,
                        chat_name: r.get(30)?,
                    })
...
#[tauri::command]
fn get_message_receipts(msg_key: String, state: State<AppState>) -> Result<Vec<MessageReceipt>, String> {
    with_db(&state, |conn| query_message_receipts_for_visible_key(conn, &msg_key))
}
...
fn message_receipt_key_filter_sql(conn: &Connection) -> Result<String, String> {
    let can_fold_protocol_receipts = table_exists(conn, "messages")?
        && ["msg_key", "chat_jid", "from_me", "timestamp", "text", "msg_type"]
            .iter()
            .map(|column| table_column_exists(conn, "messages", column))
            .collect::<Result<Vec<_>, _>>()?
            .into_iter()
...
fn query_message_receipts_for_visible_key(conn: &Connection, msg_key: &str) -> Result<Vec<MessageReceipt>, String> {
    ...
    let mut stmt = conn.prepare(&sql).map_err(|e| e.to_string())?;
    let receipts = stmt.query_map([msg_key], |r| {
            Ok(MessageReceipt {
                msg_key:        r.get(0)?,
...
fn query_message_mentions_for_keys(conn: &Connection, msg_keys: &[String]) -> Result<Vec<MessageMention>, String> {
    ...
    for chunk in msg_keys.chunks(500) {
        ...
        let params: Vec<&dyn rusqlite::types::ToSql> = chunk.iter().map(|k| k as &dyn rusqlite::types::ToSql).collect();
        let mut stmt = conn.prepare(&sql).map_err(|e| e.to_string())?;
        let rows = stmt.query_map(params.as_slice(), |r| {
            Ok(MessageMention {
                ...
                kind:          r.get::<_, Option<String>>(3)?.unwrap_or_else(|| "unknown".to_string()),
                target_jid:    r.get(4)?,
...
#[tauri::command]
fn get_message_mentions_for_messages(msg_keys: Vec<String>, state: State<AppState>) -> Result<Vec<MessageMention>, String> {
    with_db(&state, |conn| query_message_mentions_for_keys(conn, &msg_keys))
}

fn query_message_edit_history_for_key(conn: &Connection, msg_key: &str) -> Result<MessageEditHistory, String> {
    let has_edit_columns = ["is_edited", "edited_at", "edit_count", "edit_history_status"]
        .iter()
        .map(|column| table_column_exists(conn, "messages", column))
        .collect::<Result<Vec<_>, _>>()?
    ...
    let edits = if table_exists(conn, "message_edits")? {
        ...
        let rows = stmt.query_map([msg_key], |r| {
            Ok(MessageEdit {
    ...
    let quote_snapshots = if let Some(target_msg_id) = msg_id.as_deref() {
        ...
        let rows = stmt.query_map((&chat_jid, target_msg_id), |r| {
            Ok(QuoteSnapshot {
                rowid:           r.get(0)?,
                msg_key:         r.get(1)?,
                timestamp:       r.get(2)?,
...
#[tauri::command]
fn get_message_edit_history(msg_key: String, state: State<AppState>) -> Result<MessageEditHistory, String> {
    with_db(&state, |conn| query_message_edit_history_for_key(conn, &msg_key))
}

fn query_message_reactions_for_keys(conn: &Connection, msg_keys: &[String]) -> Result<Vec<MessageReaction>, String> {
...
#[tauri::command]
fn get_reactions_for_messages(msg_keys: Vec<String>, state: State<AppState>) -> Result<Vec<MessageReaction>, String> {
    with_db(&state, |conn| query_message_reactions_for_keys(conn, &msg_keys))
}

/// Batch-query receipt statuses for a list of sent message keys.
/// Returns a per-key status: "read", "delivered", or "sent".
#[tauri::command]
fn get_message_statuses(msg_keys: Vec<String>, state: State<AppState>) -> Result<Vec<MessageStatus>, String> {
    with_db(&state, |conn| query_message_statuses_for_keys(conn, &msg_keys))
}

fn query_message_statuses_for_keys(conn: &Connection, msg_keys: &[String]) -> Result<Vec<MessageStatus>, String> {
    if msg_keys.is_empty() || !table_exists(conn, "message_receipts")? {
        return Ok(Vec::new());
    }

    let mut results = Vec::with_capacity(msg_keys.len());
    for key in msg_keys {
        let receipts = query_message_receipts_for_visible_key(conn, key)?;
        if receipts.is_empty() {
            continue;
        }
        let status = if receipts.iter().any(|r| r.read_time.is_some() || r.played_time.is_some()) {
            "read"
        } else if receipts.iter().any(|r| r.delivery_time.is_some()) {
            "delivered"
        } else {
            "sent"
        };
        results.push(MessageStatus {
            msg_key: key.clone(),
...
#[tauri::command]
fn search_contacts(query: String, state: State<AppState>) -> Result<Vec<ContactSearchResult>, String> {
    with_db(&state, |conn| {
        let pattern = format!("%{}%", query);
        ...
        let results = stmt.query_map([&pattern], |r| {
...
fn query_contact_summary(conn: &Connection) -> Result<ContactSummary, String> {
    ...
    for contact in contacts {
        ...
        if contact.is_group {
            ...
            let saved = contact.contact_name.as_deref().is_some_and(|v| !v.trim().is_empty())
                || contact.short_name.as_deref().is_some_and(|v| !v.trim().is_empty());
            if saved {
                summary.saved += 1;
            } else {
                summary.unsaved += 1;
...
fn query_media_items(
    ...
    ";
    let filter_clause = match filter.unwrap_or("all") {
        "photos" => "AND (m.msg_type IN ('image','album','gif') OR m.media_mime_type LIKE 'image/%')".to_string(),
        "videos" => "AND (m.msg_type IN ('video','ptv') OR m.media_mime_type LIKE 'video/%')".to_string(),
        "audio" => "AND (m.msg_type IN ('audio','ptt') OR m.media_mime_type LIKE 'audio/%')".to_string(),
        "documents" => "AND (m.msg_type = 'document' OR m.media_mime_type LIKE 'application/%')".to_string(),
    ...
    let rows = stmt
        .query_map(rusqlite::params![needle, pattern, limit, offset], |r| {
            ...
            Ok(MediaItem {
                ...
                timestamp: r.get::<_, i64>(6).unwrap_or(0),
                text: r.get(7)?,
                media_kind: media_kind_for(msg_type.as_deref(), media_mime_type.as_deref()).to_string(),
                msg_type,
                media_mime_type,
...
fn get_messages_paginated(
    ...
    with_db(&state, |conn| {
        ...
        let select_sql = message_select_sql_owned(conn)?;
        let (sql, params_vec): (String, Vec<Box<dyn rusqlite::types::ToSql>>) = if let Some(br) = before_rowid {
        ...
        let params_slice: Vec<&dyn rusqlite::types::ToSql> = params_vec.iter().map(|p| p.as_ref()).collect();
        let mut stmt = conn.prepare(&sql).map_err(|e| e.to_string())?;
...
fn get_messages_after_rowid(
    ...
    with_db(&state, |conn| {
        ...
        .optional()
        .map_err(|e| e.to_string())?
        .ok_or_else(|| format!("Cursor message rowid {after_rowid} was not found in this chat."))?;
...
fn get_messages_around_rowid(
    ...
    with_db(&state, |conn| {
        ...
        let target_sql = message_select_sql_owned(conn)? + " WHERE m.chat_jid = ? AND m.rowid = ? LIMIT 1";
        let target = conn
            .query_row(&target_sql, rusqlite::params![chat_jid.as_str(), rowid], |r| message_from_row(r))
            .optional()
            .map_err(|e| e.to_string())?;
...
fn get_messages_by_stanza_ids(
    ...
    with_db(&state, |conn| {
        ...
        let mut global_stmt = conn
            .prepare(&(select_sql + " WHERE m.msg_id = ? ORDER BY m.timestamp ASC, m.rowid ASC LIMIT 2"))
            .map_err(|e| e.to_string())?;
        ...
        for stanza_id in stanza_ids {
            ...
            let same_chat = same_chat_stmt
                .query_row(rusqlite::params![chat_jid.as_str(), stanza_id.as_str()], |r| message_from_row(r))
                .optional()
                .map_err(|e| e.to_string())?;
            ...
            let msg = if same_chat.is_some() {
                ...
                let rows = global_stmt
                    .query_map(rusqlite::params![stanza_id.as_str()], |r| message_from_row(r))
                    .map_err(|e| e.to_string())?
                    .filter_map(|r| r.ok())
                    .collect::<Vec<_>>();
                if rows.len() == 1 {
...
mod tests {
    ...
    fn create_large_unified_db(path: &PathBuf) {
        ...
        let tx = conn.unchecked_transaction().expect("start insert transaction");
        {
            let mut stmt = tx
    ...
    fn portable_update_safety_rejects_non_exe_or_unwritable_paths() {
        ...
        let wrong_extension = portable_update_safety_for_path(
            &PathBuf::from(r"C:\Cases\WAren6-Reader.txt"),
            true,
        );
        assert!(!wrong_extension.safe);
        assert_eq!(wrong_extension.reason.as_deref(), Some("current executable is not an .exe file"));
        ...
        assert!(!blocked.safe);
        assert_eq!(blocked.reason.as_deref(), Some("current folder is not writable"));
    }
    ...
    fn builds_search_cache_for_large_unified_db() {
        ...
        let cache_path = std::env::temp_dir().join(format!("waren6-reader-cache-test-{source_hash}.sqlite"));
    ...
    fn media_kind_prefers_whatsapp_type_then_mime_family() {
        assert_eq!(media_kind_for(Some("sticker"), Some("image/webp")), "sticker");
        assert_eq!(media_kind_for(Some("interactive"), Some("image/jpeg")), "image");
    ...
    fn media_query_filters_searches_and_summarizes_items() {
        ...
        let rows = query_media_items(&conn, Some("missing"), Some("biology"), 20, 0)
            .expect("media rows");
    ...
    fn media_query_prioritizes_available_chat_media_before_missing_and_status_media() {
        ...
        let rows = query_media_items(&conn, Some("all"), Some(".jpg"), 20, 0)
            .expect("media rows");
        ...
        assert_eq!(status_filenames, vec!["status-available.jpg", "status-missing.jpg"]);
    ...
    fn contact_query_summarizes_saved_unsaved_people_and_chat_activity() {
        ...
        let contacts = query_contact_items(&conn, Some("all"), Some("example"), 50, 0)
            .expect("contact rows");
        ...
        let groups = query_contact_items(&conn, Some("groups"), None, 50, 0)
            .expect("group contact rows");
    ...
    fn contact_query_deduplicates_lid_and_phone_rows_for_same_saved_person() {
        ...
        let contacts = query_contact_items(&conn, Some("all"), Some("Saved"), 50, 0)
            .expect("contact rows");
    ...
    fn contact_query_sorts_named_latin_contacts_before_other_scripts_and_numbers() {
        ...
        let rows = query_contact_items(&conn, Some("all"), None, 50, 0)
            .expect("sorted contact rows");
    ...
    fn message_mentions_query_handles_batches_and_missing_table() {
        ...
        let rows = query_message_mentions_for_keys(&conn, &["m1".to_string(), "missing".to_string()])
    ...
    fn message_reactions_query_handles_batches_and_missing_table() {
        ...
        let rows = query_message_reactions_for_keys(&conn, &["m1".to_string()])
    ...
    fn portable_update_helper_args_include_log_file_once() {
        ...
        assert_eq!(args.last().map(String::as_str), Some(r"C:\Temp\portable-update.log"));
    }
    ...
    fn chat_query_uses_group_subject_for_missing_chat_name() {
        ...
        conn.execute_batch(
            ...
            VALUES ('120363424823211291@g.us', 'Old Group Name', 1, 0, 1778494158);
            INSERT INTO groups(group_jid, subject, participant_count)
            VALUES ('120363424823211291@g.us', 'Example Study Group', 6);
            INSERT INTO messages(rowid, msg_key, chat_jid, from_me, timestamp, text, is_group, msg_type)
            VALUES (1, 'm1', '120363424823211291@g.us', 0, 1778494158, 'Thanks...', 1, 'chat');
            ",
        )
    ...
    fn message_queries_hide_legacy_runtime_generic_duplicates() {
        ...
        let media_items = query_media_items(&conn, Some("all"), None, 20, 0).expect("query media rows");
    ...
    fn message_edit_history_query_hides_protocol_rows_and_returns_snapshots() {
        ...
        assert_eq!(history.edits[0].edit_event_msg_key.as_deref(), Some("true_12345@c.us_EDIT"));
        assert_eq!(history.quote_snapshots.len(), 1);
        assert_eq!(history.quote_snapshots[0].quoted_msg_body.as_deref(), Some("captured quote"));
    ...
    fn call_query_filters_missed_video_and_searches_contact_names() {
        ...
        let missed = query_call_items(&conn, Some("missed"), Some("example"), 50, 0)
            .expect("missed calls");
        assert_eq!(missed.len(), 1);
        assert_eq!(missed[0].call_outcome.as_deref(), Some("Missed"));

        let video = query_call_items(&conn, Some("video"), None, 50, 0)
            .expect("video calls");
